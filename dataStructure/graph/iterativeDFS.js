//1. S라는 빈 스택 만들기
//2. 함수 인자로 받은 start를 빈 스택에 넣기
//3. 이 스택이 비어 있지 않을 동안에
//4. 하나씩 pop을 한 다음
//5. 아직 방문을 하지 않았다면 discovered라는 이름의 리스트에 추가
//6. 그리고 해당 정점 인접점에 대해서 반복적으로 스택에 push
...
class Graph {
  ...
  depthFirstIterative(start) {
    const stack = [start]; //start를 stack에 push
    const result = [];
    const visited = {};
    let currentVertex;
    ...
    while (stack.length) {
      console.log(stack);
      currentVertex = stack.pop(); //다음 정점을 stack에서 pop
...
//재귀에서는 A에서 시작해서 알파벳 순서를 따라 B로 갔다.
//하지만 순환형에서는 A에서 시작하고 그 다음은 C로 간다. => ['A', 'C', 'E', 'B', 'D']

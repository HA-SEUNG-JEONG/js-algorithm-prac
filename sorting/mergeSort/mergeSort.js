...
// 한 배열이 소진되면 나머지 모든 값을 다른 어레이에서 밀어 넣기
...
const merge = (arr1, arr2) => {
  ...
  /* Comparing the first element of each array and pushing the smaller one to the result array. */
  while (arr1.length && arr2.length) {
    if (arr1[0] <= arr2[0]) result.push(arr1.shift());
    else result.push(arr2.shift());
  }
  // 남은 요소들을 모두 넣어준다.
  /* It's pushing the remaining elements of arr1 to the result array. */
  while (arr1.length) {
    result.push(arr1.shift());
  }
  /* It's pushing the remaining elements of arr2 to the result array. */
  while (arr2.length) {
...
const mergeSort = (arr) => {
  ...
  /* It's finding the middle index of the array. */
  const mid = Math.floor(arr.length / 2);

  /* It's splitting the array into two halves. */
  ...
  /* Calling mergeSort on the left and right halves of the array, and then merging the results. */
